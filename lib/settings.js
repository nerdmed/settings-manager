...
SettingsManager = function(obj) {
    ...
    this.fixed = false;
    this.save = obj.save || "session";
    this.init = obj.init;
    this.rawHelper = obj.rawHelper || false;

    // Crate the reactive Dict - Pesistant Reactive Dict if set
    if(this.save === "session") this.save = "temporary";
    if(this.save !== "temporary" && this.save !== "persistent" && this.save !== "authenticated" ){
        throw new Meteor.Error(003, 'Error 004: You forgot to select save method that is either session (temporary), persistent or authenticated');
    }

    this.reactiveDict = new PersistentReactiveDict(this.name, this.save);



    if (obj.fixed) this.fixed = obj.fixed;
    ...
    if (obj.init != undefined && typeof obj.init == "object") {
        ...
        // only init settings that we dont have already have in memory
        _.each(obj.init, function(value, key, initSets){
            if( !self.get(key) ) self.set(key, value); 
        });
