...
_.extend(Setting.prototype, {
    ...
    _afterSet: function(old, n) {
        this._runCallbacks('afterSet', old, n);
    },
    _onChange: function(old, n) {
        this._runCallbacks('onChange', old, n);
    },
...
_.extend(SettingsManager.prototype, {
    ...
    _setValue: function(name, newValue) {
        ...
        this._beforeSetAll.call(setting, oldValue, newVal);

        setting._beforeSet.call(setting, oldValue, newVal);

        workResult(setting, oldValue, newVal, function(oldval, newval, result) { // xxx work of result in a different function to set new Values
...
workResult = function(setting, oldval, newval, fkt) {
    // works of result and start new function xxx

    var newValue = newval;

    if (setting.changedValue  != undefined) {
        newValue = setting.changedValue;
        setting.changedValue = null;
    }
